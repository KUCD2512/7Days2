...
if ((_vehicle != player) && (isEngineOn _vehicle)) then {
	if (driver _vehicle == player) then {
		...
		if (_hasNitro == 1 && NitroBoostOn) then {		
			if (isNil 'NitroMessage') then {
				NitroMessage = true;
				systemChat localize "STR_ACTIVATED_NITRO_BOOST";
			};
			if (_dikCode == 0x2A) then {				
				...
				if (isNil 'NitroSound') then {
					NitroSound = true;
					[nil,_vehicle, rSAY,["nitro", 50]] call RE;
					/*_vehicle spawn {						
						NitroSound = true;
						[nil,_this, rSAY,["nitro", 50]] call RE;
						uisleep 0.2;
						NitroSound = nil;							
					};*/
				};				
