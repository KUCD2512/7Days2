private ["_targetPos","_rockets","_launcher","_type","_skin","_gain","_mission","_ainum","_unit","_player","_humanity","_banditkills","_humankills","_humanitygain","_cash"];
	...
	if (ai_share_info) then {
		{
			if (((position _x) distance (position _unit)) <= ai_share_distance) then {
				...
				_targetPos = ASLtoATL getPosASL _player;
				_x doWatch _targetPos;
				_x doMove _targetPos;
				_x doTarget _player;
				_x doFire _player;
				_x commandFire _player;
				_x commandTarget _player;
			};
		} count allUnits;		
	};
