...
//----------- CONFIG --------------------------
_keepVehicle = false;                             // save vehicle in database and put key in unit
...
_fugiWeaponClass = "HK416_CCO_SD_DZ";               // class name fugitive weapon
_fugiWeaponAmmo = "30Rnd_556x45_StanagSD";                // class name of the ammo for the fugitive weapon
_numberMagsWeapon = 4;                           // number magazine for the fugitive weapon
_fugiLuncherClass = "L110A1_Holo_DZ";                      // clssname of the luncher
_fugiLuncherAmmo = "200Rnd_556x45_L110A1";                       // clssname ammo of the luncher
...
_vehicle_loot = [["forest_large_net_kit",1],["cinder_garage_kit",2],["CinderBlocks",10],["ChainSaw",1],["M249_EP1_DZ",1],["MG36_DZ",2]];    // loot added inside vehicle
_coinOnFugitive = true;                            // put coins on fugutive (true / false)
_fugitiveCoins = 1500;                          // number  Coin in the fugitive ,if you want a random amount : round(random 20) * 1000; // number between 0 and 20 000
...
_waitTime          = 1000;                       // time end event                    // time end event
//------------END CONFIG ---------------------------
...
_monitor = {
	...
	while {alive _unit} do {
		...
		if ((timeleft >= 300) and (timeleft <= 325) and (isNil "stepToo")) then { // ~5 minutes
		    stepToo = true;
			[nil,nil,rTitleText, "The fugitive to find a MG and ammunition !", "PLAIN",10] call RE;
		...
		if ((timeleft >= 600) and (timeleft <= 660) and (isNil "stepFour")) then {   // ~10 minutes
			...
			car = createVehicle ["HMMWV_M1151_M2_CZ_DES_EP1_DZ",_posCar, [],0, "NONE"];
